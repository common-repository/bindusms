...
class WCOrderNotification
{
	...
	public function send_new_order_notification($order_id)
	{
		...
		$text       = esc_sanitize(bindusms_process_order_message($settings['new_order_notification_message'], $order_details));
		$to         = esc_sanitize(bindusms_process_phone_number($settings['new_order_notification_phone']));
		$from       = esc_sanitize($settings['from']);

		bindusms_send_sms(array(
			'api_key'       => $api_key,
			'from'          => $from,
			'to'            => $to,
			'text'          => $text,
	...
	public function wc_order_status_change_handler($order_id)
	{
		...
		$api_key    = esc_sanitize($settings['api_key']);
		$text       = esc_sanitize(bindusms_process_order_message($settings[$current_status], $order_details));
		$to         = esc_sanitize(bindusms_process_phone_number($order_details->get_billing_phone()));
		$from       = esc_sanitize($settings['from']);
		...
		$response = bindusms_send_sms($body);

		// send debug if enabled
		if ($settings['debug_enabled'] && $settings['debug_email']) {
			bindusms_send_debug_mail($response, $settings['debug_email'], $order_id, $from, $to, $text);
		}
	}
	...
	{
		global $bindusms_wc_order_notification_settings;
		...
		bindusms_wc_update_order_notification_settings($data);

		$redirect_to = admin_url('admin.php?page=bindusms&saved=true');
