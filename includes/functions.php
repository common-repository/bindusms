...
function bindusms_wc_update_order_notification_settings($args = [])
{
	update_option(BINDUSMS_WC_ORDER_NOTIFICATION_SETTINGS_KEY, $args);
}

function bindusms_send_sms($args = [])
{
	$url = "https://smsapi.bindulogic.com/send-sms";
	$data = array(
		'to'            => $args['to'],
		'senderId'		=> $args['from'],
		'message'          => $args['text'],
	...
	return wp_remote_get($query_url);
}
 ...
 * @return void
 */
function bindusms_send_debug_mail($response, $email, $order_id, $from, $to, $text)
{
	$subject = 'WC BinduSms SMS Debug for #' . $order_id;

	if (is_wp_error($response)) {
		wp_mail($email, $subject, 'Error: Something went wrong on the BinduSms server or WP.');
	} else {
		...
		$message .= '<br /><br />API response from the BinduSms server:';
		$message .= '<br /><pre>' . json_encode(json_decode($response['body']), JSON_PRETTY_PRINT) . '</pre>';
		$message .= '<br /><p>Regards,<br />The BinduSms Team</p>';
 ...
 * @return string
 */
function bindusms_process_phone_number($phone_number)
{
 ...
 * @return string
 */
function bindusms_process_order_message($message, $order_details)
{
