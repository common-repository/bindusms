...
class Installer
{
	...
	{
		$installed = get_option('bindusms_installed');

		if (!$installed) {
			update_option('bindusms_installed', time());
		}

		update_option('bindusms_version', BINDUSMS_VERSION);
	}

	public function set_wc_order_notification_settings()
	{
		$settings = get_option('bindusms_wc_order_notification_settings');
		...
		if (!$settings) {
			...
			update_option(BINDUSMS_WC_ORDER_NOTIFICATION_SETTINGS_KEY, $defaultSettings);
		}
	}
}
